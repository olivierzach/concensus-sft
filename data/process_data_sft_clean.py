...
def main() -> None:
    ...
    parser.add_argument("--min_oracle_rougeL", type=float, default=0.15)
    ...
    parser.add_argument("--append_end_token", action="store_true")
    parser.add_argument("--end_token", default="[END]")
    ...
    df["overlap"] = [overlap_ratio(l, a) for l, a in zip(df["label"], df["abstract_trimmed"])]

    oracle_texts = []
    oracle_scores = []
    for query, label, abstract in zip(df["query"], df["label"], df["abstract_trimmed"]):
    ...
    df = df[df["oracle_rougeL"] >= args.min_oracle_rougeL]
    df = df[df["oracle_text"].str.len() > 0]
    after = len(df)
    ...
    if args.append_end_token:
        df["target_text"] = df["target_text"].astype(str).str.rstrip() + f" {args.end_token}"
